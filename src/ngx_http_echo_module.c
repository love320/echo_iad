...
static char * ngx_http_echo_echo(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static char * ngx_http_echo_iad(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
...
static char * ngx_http_echo_iad(ngx_conf_t *cf, ngx_command_t *cmd,void *conf)
{
    ngx_http_echo_loc_conf_t        *elcf = conf;
    ...
    dd("in echo_echo...");
    return ngx_http_echo_helper(echo_opcode_echo_iad, echo_handler_cmd,
                                cf, cmd, conf);
}
...
ngx_http_echo_post_config(ngx_conf_t *cf)
{
    ...
    rc = ngx_http_echo_iad_init(cf);//加入iad 初始化
    if (rc != NGX_OK) {
        return rc;
    }
